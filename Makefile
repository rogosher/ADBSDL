...
CXXFLAGS_STD     = -std=gnu++11 -fpermissive -fno-exceptions \
		   -ffunction-sections -fdata-sections -fno-threadsafe-statics \
		   -MMD \

CXXFLAGS        += $(CXXFLAGS_STD) -pedantic
...
ARDUINO_LIBS = -I/var/tmp/Arduboy/arduino-nightly/hardware/arduino/avr/cores/arduino \
		...
		-I/var/tmp/Arduboy/arduino-nightly/hardware/tools/avr/avr/include/avr/pgmspace.h \
...
AVR_SOURCES = sfr_defs.h io.h power.h

AVR_INCLUDES = $(addprefix -include , $(AVR_SOURCES))

$(call print_output,$(AVR_INCLUDES))

ARDUINO_FLAGS = -DF_CPU=16000000L -DARDUINO=night \
		-DUSB_VID=0x2341 -DUSB_PID=0x8036 \
		-DARDUINO_AVR_LEONARDO -DARDUINO_ARCH_AVR \
		-D__PROG_TYPES_COMPAT__ \
		-D__AVR_ATmega32U4__

CPPFLAGS += $(ARDUINO_FLAGS) -I/usr/lib/avr/include/avr/ $(ARDUBOY_LIBS) $(AVR_INCLUDES) 
...
# recursive wildcard
# see https://github.com/sudar/Arduino-Makefile for the original implementation
...
LOCAL_CPP_SRCS ?= $(wildcard $(ARDUBOY_LIB)/*.cpp)
...
$(OBJDIR)/%.cpp.o: $(ARDUBOY_USER_LIB)/%.cpp 
	@$(MKDIR) $(dir $@)
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -c $< -o $@

$(OBJDIR)/%.c.o: $(ARDUBOY_USER_LIB)/%.c
	@$(MKDIR) $(dir $@)
	$(CXX) $(CPPFLAGS) $(CXXGLAGS)-c $< -o $@
